...
class Account:
    def __init__(self, name=None, password=None):
        ...
        # 配置重试策略：遇到 500/502/503/504 错误时自动重试 3 次，间隔极短
        ...
        while True:
            try:
                ...
                if image_response.ok:
                    break
                print(f"Get captcha failed (status {image_response.status_code}), retrying...")
                time.sleep(1)
            except requests.exceptions.RequestException as e:
                print(f"Get captcha error: {e}, retrying...")
                time.sleep(2)
    ...
    def login(self):
        ...
        while True:
            try:
                ...
                if init_response.ok:
                    ...
                    print(f"##GET login page failed (status {init_response.status_code}), retrying...")
                    time.sleep(1)
            except requests.exceptions.RequestException as e:
                print(f"##GET login page error: {e}, retrying...")
                time.sleep(2)
        ...
        while True:
            try:
                ...
                if login_response.ok:
                    break
                print(f"Login POST failed (status {login_response.status_code}), retrying...")
                time.sleep(1)
            except requests.exceptions.RequestException as e:
                print(f"Login POST error: {e}, retrying...")
                time.sleep(2)
        ...
        while try_time < 300:
            try:
                ...
                print(f"Login check GET error: {e}, retrying...")
                time.sleep(1)
                continue
